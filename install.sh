...
# Silient generate SSL cert
	openssl req -nodes -newkey rsa:2048 -keyout postfix/server.key -out postfix/server.crt -subj "/C=RU/ST=Name Surname/L=Location/O=Company/OU=Department/CN=$domain"
...
# Create mysql user for mail
	...
	echo "CREATE DATABASE mail;" | mysql -p$mysqlrootpasswd mysql > /dev/null 2>&1 || echo -e "\e[31m\t>> create database failed\e[0m";
